...
# Compatible means tasks-axi --version parses as a semver at all (a fail-closed
# precondition: an unparseable version reports incompatible before either probe
# runs), `tasks-axi update --help` exposes --archive-body for recoverable note
# rewrites, and `tasks-axi mv --help` exposes [<id>...] for atomic multi-ID moves
# required by secondmate handoffs. That mv probe is mandatory and multi-ID mv
# landed in tasks-axi 0.2.2, so the effective floor is 0.2.2 or newer.
